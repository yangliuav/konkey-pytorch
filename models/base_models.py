...
class BaseModel(torch.nn.Module):
    ...
    Models inheriting from `BaseModel` can be used by :mod:`konkey.separate`
    and by the `konkey-infer` CLI. For models whose `forward` doesn't go from
    ...
    def serialize(self):
        ...
        from __init__ import __version__ as konkey_version  # Avoid circular imports
...
class BaseEncoderMaskerDecoder(BaseModel):
    ...
    Args:
        ...
        encoder_activation (Optional[str], optional): Activation to apply after encoder.
            See ``konkey.masknn.activations`` for valid values.
    """
