...
class SingleSrcMSE(_Loss):
    ...
    def forward(self, est_targets, targets):
        return MSE(est_targets, targets)

def MSE(est_targets, targets):
    r"""Measure mean square error on a batch.
    Supports both tensors with and without source axis.
    ...
    Examples
        ...
        >>> loss = MSE(est_targets, targets)
    """
    if targets.size() != est_targets.size() or targets.ndim < 2 or targets.ndim > 3:
        raise TypeError(
    ...
    if targets.ndim == 2:
        ...
        est_targets = est_targets.view(est_targets.shape[0], 1, est_targets.shape[1])
